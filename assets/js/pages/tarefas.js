...
function Filter() {
  ...
  tarefas.forEach((tarefa) => {
    ...
    const titulo = tarefa
      .querySelector(".task-description h3")
      .innerText.toLowerCase();
    const descricao = tarefa
      .querySelector(".task-description p")
      .innerText.toLowerCase();
    const prioridade = tarefa
      .querySelector(".priority-tag p")
      .innerText.toLowerCase();
    ...
    const temTexto =
      titulo.includes(tipoPesquisa) || descricao.includes(tipoPesquisa);
...
function setCustomSelectValue(select, value) {
  ...
  options.forEach((opt) => {
    ...
    if (opt.dataset.value === value) {
      opt.classList.add("selected");
      select.querySelector(".custom-select-text").textContent =
...
function excluirTarefaGlobal(event) {
  ...
  if (card) {
    if (confirm("Deseja realmente excluir esta tarefa?")) {
      ...
      // Fazer requisição ao servidor para deletar a tarefa do banco
      fetch(`/pi2-escopo-facil/controllers/excluir-tarefa.php?id=${tarefaId}`, {
        method: "GET",
      })
        .then((response) => {
          if (response.ok) {
            // Se a requisição foi bem-sucedida, remove o card do DOM
            card.remove();
          } else {
